...
public class InventarioForm extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jPanel2.add(btnSalir, new org.netbeans.lib.awtextra.AbsoluteConstraints(270, 400, 140, 40));
        ...
        jPanel2.add(btnLimpiar, new org.netbeans.lib.awtextra.AbsoluteConstraints(500, 340, 140, 40));

        jPanel1.add(jPanel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(700, 100, 670, 450));
...
registrarAccion("Eliminó el registro de inventario del producto ID: " + txtId_Producto.getText());
  ...
  // 1. Validación básica de campos vacíos
    if (txtId_Producto.getText().isEmpty() || txtStock_Actual.getText().isEmpty()) {
        JOptionPane.showMessageDialog(null, "⚠️ Por favor, complete el ID del Producto y el Stock.");
        return;
    }

    try {
        int idProd = Integer.parseInt(txtId_Producto.getText().trim());
        
        // --- CORRECCIÓN: Verificación manual de existencia del producto ---
        boolean existe = false;
        try (java.sql.Connection conVerificar = db.Conexion.conectar();
             java.sql.PreparedStatement psVerificar = conVerificar.prepareStatement("SELECT id_producto FROM producto WHERE id_producto = ?")) {
            psVerificar.setInt(1, idProd);
            java.sql.ResultSet rs = psVerificar.executeQuery();
            if (rs.next()) {
                existe = true;
            }
            registrarAccion("Ajustó inventario del producto ID: " + idProd);
        }

        if (!existe) {
            JOptionPane.showMessageDialog(null, "❌ Error: El Producto con ID " + idProd + " no existe.\nVerifica en la tabla de productos.");
            return; // Detenemos la ejecución aquí para evitar el error de MySQL
        }
        // ------------------------------------------------------------------

        // 2. Si existe, procedemos a llenar el objeto y guardar
        inv.setId_producto(idProd);
        inv.setStock_total(Integer.parseInt(txtStock_Actual.getText().trim()));
        ...
        JOptionPane.showMessageDialog(null, "⚠️ Error: El ID y el Stock deben ser números.");
    } catch (java.sql.SQLException e) {
...
// 1. Verificar que haya un registro seleccionado
    if (txtId.getText().isEmpty()) {
        JOptionPane.showMessageDialog(null, "⚠️ Seleccione un registro de la tabla para modificar.");
        return;
    }
    ...
    try {
        ...
        inv.setId_producto(Integer.parseInt(txtId_Producto.getText().trim()));
        inv.setStock_total(Integer.parseInt(txtStock_Actual.getText().trim()));
        inv.setNombre_producto(txtFecha_Ingreso.getText());
        inv.setUbicacion(txtDireccion.getText());
         
        registrarAccion("Modificó stock del producto ID: " + txtId_Producto.getText());
        
        // 3. Ejecución en el DAO
        ...
        JOptionPane.showMessageDialog(null, "⚠️ Error: Verifique que ID y Stock sean números.");
    }
